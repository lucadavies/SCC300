...
\subsubsection*{Hi Paul, obviously this won't be in the final report! :) \\ RFC on the below if I'm missing anything}
    \begin{itemize}
        ...
        \item Paul's Recommendations
            \begin{itemize}
                \item Add to above section regarding an overview of available NLP/SA toolkits
                \item Explain why CoreNLP was slow sec Top level language
                \item Justify use of waterfall/agile sec design/dev
                \item Create / include design diagram for Sentiplot sec design/dev
                \item Add summary to sec Sentiplot Tool (or UI... (or ResDisplay...))
                \item Add overview of sec Experimentation
                \item TF is ``vard'', actually explain it!!
                \item Consider what to do about the buggered input corpora from Gutenberg (re: section \ref{subsec:EMvME} and different length/block size)
                \item Move Gutenberg format chapter to a new section `Data' between current 3/4
                \begin{itemize}
                    \item Describe whole dataset
                    \item Why these corpora?
                \end{itemize}
                \item Conclusion intro needs to be much better (duh you wrote it while basically asleep)
                \item Elaborate on Sentiplot success in conclusion
                \item Reflection: SA in general as a method?
                \item Reflection: Actual execution, what to do differently
                \item Be bolder in closing statement, you have actually done some cool stuff!
            \end{itemize}
    \end{itemize}
...
\section{Introduction}
    \subsection{Overview}
        ...
        With the high-level view clarified, it's important to note the tools within natural language processing that are relevant, namely that in this study, sentiment analysis takes the spotlight. Emotional analysis is a more in-depth field that shifts focus toward the psychological analysis \citep{sentimentVsEmotionAnalysis} and employs machine learning and artificial intelligence to further predict and understand the emotions presented in text.
...
\label{sec:background)}
    ...
    \subsection{Natural Language Processing \& Sentiment Analysis}
        ...
        With both NLP and SA garnering more and more interest in both industrial academic sectors, there are many all-in-one tools comprising both the NLP features required to perform SA but tools for conducting SA itself.
    \subsection{Kurt Vonnegut on ``The Shape of Stories''}
        ...
        \begin{figure}
            ...
            \caption{Vonnegut's Story Types \citep{vonnegutThesis}}
...
\newpage
\section{Data}
    Blah blah blah data from project Gutenberg etc blah blah blah
    \subsection{Selected Corpora}
        Some stuff was chosen ooooh.
